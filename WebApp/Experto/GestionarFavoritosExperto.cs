    ...
    public static class GestionarFavoritosExperto
    {
        public static List<DTOFavorito> ListarFavoritos(string usuarioId) {
            ...
            using (var db = new ApplicationDbContext()) {
                List<Favorito> favoritos = favoritoRepo.Buscar(x => x.userId == usuarioId && x.activo==true,db);
            ...
            using (var db = new ApplicationDbContext()) {
                using (var database = db.Database.BeginTransaction()) {
                    ...
                    try
                    {
                        ...
                        Repository<Favorito> favoritoRepo = new Repository<Favorito>();
                        Favorito favorito = favoritoRepo.Buscar(x => x.publicacionId == publicacionID && x.userId == usuarioID, db).FirstOrDefault();
                        if (favorito != null)
                        {
            ...
            using (var db = new ApplicationDbContext()) {
                using (var database = db.Database.BeginTransaction()) {
                    try
                    {
                        ...
                        if (favoritoRepo.Buscar(x => x.userId == usuarioId && x.publicacionId == publicacionId && x.activo==true, db).Count > 0)
                        {
                            error.codigoError = 1;
                            return error;
                        }
