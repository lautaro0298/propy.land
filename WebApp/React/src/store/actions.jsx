...
export const getAllHotel = (query = null, currPage = 1) => dispatch => {
  ...
  dispatch(requestAction);
    ...
    return axios.get(`http://localhost:5000/api/Busqueda/obtenerPropiedadesParaEvaluarBusqueda`)
    .then((res) => {
...
export const priceFilter = (payload, query) => dispatch => {     
    ...
    axios.get('http://localhost:5000/api/Busqueda/buscardortipo?'+tipoPropiedadId + caracteristicaIds).then((res) => {
            let { data } = res;
            console.log(data);
...
export const addHotelList = (cant ,moneda ) => dispatch => {
    axios.get('http://localhost:5000/api/Busqueda/buscardorPorPrecio?cant=' + cant + '&moneda=' + moneda).then((res) => {
        let { data } = res;
        dispatch(hotelSuccess(data));
    })
}
export const tipoPublicante = (tipoPublicante) => dispatch => {
    axios.get('http://localhost:5000/api/Busqueda/buscardorPorPublicancion?Publicacion='  + tipoPublicante).then((res) => {
            let { data } = res;
            dispatch(hotelSuccess(data));
        })
}
export const fliterDormitorios = (values) => dispatch => {
    ...
    axios.get('http://localhost:5000/api/Busqueda/buscardorAmbientes?cantCocheras=' + cocheras + '&caracteristicaId=' + habitaciones).then((res) => {
        let { data } = res;       
...
export const sortHotelData = (tipoPropiedad) => dispatch => {
  dispatch(hotelRequest())
    ...
    axios.get(`http://localhost:5000/api/Busqueda/buscardortipoPropiedad?tipoPropiedadId=` +tipoPropiedad).then((res) => {
      let { data } = res;
      dispatch(hotelSuccess(data));
    }).catch(err => {
...
export const palabra = (Palabra) => dispatch => {
    ...
    if (Palabra == undefined || Palabra == null || Palabra == "") {
        axios.get(`http://localhost:5000/api/Busqueda/obtenerPropiedadesParaEvaluarBusqueda`)
        .then((res) => {
            dispatch(hotelSuccess(res.data));
        })
        .catch((err) => {
        ...
        axios.get(`http://localhost:5000/api/Busqueda/buscardorPorPalabra?Palabra=` + Palabra).then((res) => {
            let { data } = res;
            dispatch(hotelSuccess(data));
        }).catch(err => {
            dispatch(hotelFailure(err, 1));
